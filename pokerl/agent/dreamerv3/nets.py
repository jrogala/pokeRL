...
class RSSM(nj.Module):
    def __init__(
        self, deter=1024, stoch=32, classes=32, unroll=False, initial="learned", unimix=0.01, action_clip=1.0, **kw
    ):
        self._deter = deter
        self._stoch = stoch
...
class MLP(nj.Module):
    def __init__(self, shape, layers, units, inputs=None, dims=None, symlog_inputs=False, **kw):
        if inputs is None:
            inputs = ["tensor"]
    ...
    def _layer(self, x):
        if self._transp:
            ...
            x = jax.lax.conv_transpose(
                x, kernel, (self._stride, self._stride), self._pad, dimension_numbers=("NHWC", "HWOI", "NHWC")
            )
        else:
            ...
            x = jax.lax.conv_general_dilated(
                x, kernel, (self._stride, self._stride), self._pad, dimension_numbers=("NHWC", "HWIO", "NHWC")
            )
        if self._bias:
...
class Linear(nj.Module):
    def __init__(
        self, units, act="none", norm="none", bias=True, outscale=1.0, outnorm=False, winit="uniform", fan="avg"
    ):
...
class Input:
    def __init__(self, keys=None, dims=None):
        if keys is None:
            keys = ["tensor"]
